...
import {
  ...
  salirDeFamilia, unirseAFamilia, type EspacioFamilia, type MiembroFamilia,
  type MovimientoFamilia,
} from "@/utils/cloudFamilia";
import { ArrowDown, ArrowUp, Check, LogOut, Plus, RefreshCw, Trash2, UserPlus, UsersRound, X } from "lucide-react-native";
...
export default function Family() {
  const { t, fmt, userName, showToast } = useAppData();
  ...
  const invitar = () => ejecutar(async () => {
    const uid = auth.currentUser?.uid; if (!uid || !familia) return;
  ...
  return <View className="flex-1 bg-white dark:bg-noche" style={{ paddingTop: insets.top + 6, paddingBottom: insets.bottom }}>
    ...
    <ScrollView className="flex-1 px-5" contentContainerStyle={{ paddingBottom: 36 }} keyboardShouldPersistTaps="handled">
      {cargando ? <Text className="py-8 text-center text-slate-500">{t("common.loading")}</Text> : !auth.currentUser ? <Text className="mt-6 text-center text-slate-600 dark:text-slate-300">{t("family.loginRequired")}</Text> : !familia ? <>
        ...
        <View className="mt-4 flex-row gap-3"><TouchableOpacity onPress={() => setModo("crear")} className="min-h-12 flex-1 flex-row items-center justify-center gap-2 rounded-2xl bg-emerald-600"><Plus size={18} color="#fff" /><Text className="font-bold text-white">{t("family.create")}</Text></TouchableOpacity><TouchableOpacity onPress={() => setModo("unir")} className="min-h-12 flex-1 flex-row items-center justify-center gap-2 rounded-2xl bg-slate-100 dark:bg-noche-2"><UserPlus size={18} color="#0d9488" /><Text className="font-bold text-teal-700 dark:text-teal-300">{t("family.join")}</Text></TouchableOpacity></View>
        ...
        <View className="mt-2 rounded-3xl bg-emerald-600 px-5 py-5"><Text className="font-bold text-emerald-100">{familia.nombre}</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.65} className="mt-1 text-3xl font-extrabold text-white">{fmt(saldo)}</Text><Text className="mt-1 text-xs text-emerald-100">{t("family.sharedBalance")}</Text></View>
        ...
        <View className="mt-4 rounded-2xl border-[1.5px] border-slate-200 p-3 dark:border-noche-borde"><View className="flex-row items-center justify-between"><Text className="font-extrabold text-slate-900 dark:text-slate-100">{t("family.members")} · {miembros.length}</Text><TouchableOpacity onPress={invitar} className="min-h-10 flex-row items-center gap-1 rounded-xl bg-teal-50 px-3 dark:bg-teal-950"><UserPlus size={16} color="#0d9488" /><Text className="text-xs font-bold text-teal-700 dark:text-teal-300">{t("family.invite")}</Text></TouchableOpacity></View>{invitacion ? <View className="mt-3 rounded-xl bg-emerald-50 p-3 dark:bg-emerald-950"><Text className="text-xs text-slate-600 dark:text-slate-300">{t("family.shareCode")}</Text><Text selectable className="mt-1 text-center text-2xl font-extrabold tracking-[4px] text-emerald-700 dark:text-emerald-300">{invitacion}</Text><Text className="mt-1 text-center text-[11px] text-slate-500">{t("family.codeExpires")}</Text></View> : null}<View className="mt-2 flex-row flex-wrap gap-2">{miembros.map(item => <View key={item.uid} className="rounded-full bg-slate-100 px-3 py-1.5 dark:bg-noche-2"><Text className="text-xs font-bold text-slate-700 dark:text-slate-200">{item.nombre}{item.rol === "owner" ? " · ★" : ""}</Text></View>)}</View></View>
        <Text className="mb-2 mt-5 font-extrabold text-slate-900 dark:text-slate-100">{t("family.history")}</Text>
        {movimientos.length === 0 ? <Text className="py-5 text-center text-sm text-slate-500">{t("family.noMovements")}</Text> : movimientos.map(item => <View key={item.id} className="mb-2 flex-row items-center rounded-2xl border-[1.5px] border-slate-200 p-3 dark:border-noche-borde"><View className={`h-9 w-9 items-center justify-center rounded-xl ${item.tipo === "ingreso" ? "bg-emerald-100" : "bg-rose-100"}`}>{item.tipo === "ingreso" ? <ArrowUp size={17} color="#047857" /> : <ArrowDown size={17} color="#be123c" />}</View><View className="ml-3 flex-1"><Text numberOfLines={1} className="font-bold text-slate-800 dark:text-slate-100">{item.descripcion || t(item.tipo === "ingreso" ? "boxes.income" : "boxes.expense")}</Text><Text className="text-[11px] text-slate-500">{item.fecha}</Text></View><Text className={`mr-1 font-extrabold ${item.tipo === "ingreso" ? "text-emerald-600" : "text-rose-600"}`}>{item.tipo === "ingreso" ? "+" : "-"}{fmt(item.monto)}</Text><TouchableOpacity onPress={() => void borrar(item.id)} className="h-10 w-10 items-center justify-center"><Trash2 size={16} color="#e11d48" /></TouchableOpacity></View>)}
