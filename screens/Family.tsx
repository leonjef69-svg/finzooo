  ...
  return <View className="flex-1 bg-white dark:bg-noche" style={{ paddingTop: insets.top + 6, paddingBottom: insets.bottom }}>
    ...
    <ScrollView className="flex-1 px-5" contentContainerStyle={{ paddingBottom: 36 }} keyboardShouldPersistTaps="handled">
      {cargando ? <Text className="py-8 text-center text-slate-500">{t("common.loading")}</Text> : !auth.currentUser ? <Text className="mt-6 text-center text-slate-600 dark:text-slate-300">{t("family.loginRequired")}</Text> : !familia ? <>
        ...
        <View className="mt-2 rounded-3xl bg-emerald-600 px-5 py-4"><Text className="text-base font-bold text-emerald-100">{familia.nombre}</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.58} className="text-[28px] font-extrabold text-white">{fmt(saldo)}</Text><Text className="text-xs text-emerald-100">{t("family.sharedBalance")}</Text><View className="mt-2 flex-row border-t border-emerald-400/60 pt-2"><View className="flex-1"><Text className="text-xs font-semibold text-emerald-100">{t("boxes.income")}</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.65} className="text-sm font-extrabold text-white">{fmt(resumen.ingresos)}</Text></View><View className="mx-3 h-8 w-px bg-emerald-400/70" /><View className="flex-1"><Text className="text-xs font-semibold text-emerald-100">{t("boxes.expense")}</Text><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.65} className="text-sm font-extrabold text-white">{fmt(resumen.gastos)}</Text></View></View></View>
