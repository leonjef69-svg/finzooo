    ...
    >
      {!uid ? <Text className="text-slate-600 dark:text-slate-200">{t("boxes.loginRequired")}</Text> : <>
        {!caja ? <>
          ...
          <View className="mb-3 flex-row items-center justify-between gap-2"><TouchableOpacity accessibilityRole="button" accessibilityLabel="Mostrar todos los movimientos de caja" onPress={() => setFilter(null)} className="min-w-0 flex-1"><Text numberOfLines={2} className={`text-[13px] font-extrabold ${filter === "ingreso" ? "text-emerald-700 dark:text-emerald-300" : filter === "gasto" ? "text-rose-700 dark:text-rose-300" : "text-slate-900 dark:text-white"}`}>Movimientos de caja</Text></TouchableOpacity><TouchableOpacity accessibilityLabel="Filtrar ingresos de caja" onPress={() => setFilter("ingreso")} className={`h-8 w-8 items-center justify-center rounded-xl ${filter === "ingreso" ? "bg-emerald-200" : "bg-emerald-50"}`}><ArrowUp size={17} color="#047857" /></TouchableOpacity><TouchableOpacity accessibilityLabel="Filtrar gastos de caja" onPress={() => setFilter("gasto")} className={`h-8 w-8 items-center justify-center rounded-xl ${filter === "gasto" ? "bg-rose-200" : "bg-rose-50"}`}><ArrowDown size={17} color="#be123c" /></TouchableOpacity>{caja.ownerUid === uid ? <TouchableOpacity accessibilityLabel="Invitar a esta caja" disabled={busy} onPress={() => { if (!isPremium) { irUnaVez("/premium"); return; } void ejecutar(async () => setInvitacion(await crearInvitacionCaja(uid, caja.id))); }} className="h-8 w-8 items-center justify-center rounded-xl bg-teal-50 dark:bg-teal-950"><UserPlus size={16} color="#0d9488" /></TouchableOpacity> : null}</View>
          {invitacion ? <View className="mb-3 rounded-xl bg-emerald-50 p-3 dark:bg-emerald-950"><View className="flex-row items-center justify-between"><Text className="text-xs text-slate-600 dark:text-slate-300">Código de invitación</Text><TouchableOpacity accessibilityLabel="Ocultar código" onPress={() => setInvitacion("")} className="h-9 w-9 items-center justify-center"><X size={18} color="#64748b" /></TouchableOpacity></View><Text selectable className="mt-1 text-center text-2xl font-extrabold tracking-[4px] text-emerald-700 dark:text-emerald-300">{invitacion}</Text><Text className="mt-1 text-center text-[11px] text-slate-500">Mantén presionado el código para copiarlo.</Text></View> : null}
