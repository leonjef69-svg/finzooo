...
export default function FamilySettings() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      const actual = listaFamilias.find(item => item.id === familyId) ?? listaFamilias[0] ?? null;
  ...
  const cerrar = () => Alert.alert(t("family.close"), t("family.closeWarning"), [
    ...
    { text: t("family.close"), style: "destructive", onPress: () => void (async () => {
      if (!familia) return;
      try { await cerrarFamilia(uid, familia.id); safeBack(); } catch { showToast(t("family.closeBalance")); }
    })() },
  ]);

  return <View className="flex-1 bg-white dark:bg-noche" style={{ paddingTop: insets.top + 6, paddingBottom: insets.bottom }}>
    <View className="flex-row items-center px-5 pb-4"><BackButton onPress={safeBack} /><Text className="ml-3 flex-1 text-lg font-extrabold text-slate-900 dark:text-slate-100">Administrar familia</Text></View>
    <ScrollView className="flex-1 px-5" contentContainerStyle={{ paddingBottom: 36 }} keyboardShouldPersistTaps="handled">
      {cargando ? <Text className="py-8 text-center text-slate-500">Cargando…</Text> : !familia ? <Text className="py-8 text-center text-slate-500">No hay una familia activa.</Text> : <>
